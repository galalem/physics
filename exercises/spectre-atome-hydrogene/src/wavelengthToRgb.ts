...
// Outside the visible band (< 380 nm UV, > 780 nm IR) we return dim greys so the
// spectrum strip can still colour the invisible transitions with a legible marker.
export function wavelengthToRgb(nm: number): [number, number, number] {
  ...
  if (nm >= 380 && nm < 440) {
    ...
    // UV placeholder: dim violet
    return [70, 40, 110]
  } else {
    // IR placeholder: dim dark red
    return [90, 25, 25]
  }
  let factor = 1
