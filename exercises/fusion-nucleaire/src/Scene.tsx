...
// Left panel — plasma view of un-fused D–T pairs
const GRID_X = 32
const GRID_Y = 60
const GRID_W = 340
const GRID_H = 340

// Right panel — un-fused population N(t)
const PLOT_X = 400
const PLOT_Y = 60
const PLOT_W = 300
const PLOT_H = 340

// Fusion half-life slider (arbitrary "time units", u).
// T½ᶠᵘˢ is a monotone proxy for plasma temperature via the Gamow factor —
// see the concept text on stage 1.
const T_MIN = 5
const T_MAX = 40
const T_DEFAULT = 20

// Sim clock ceiling — ≈ 3× T_MAX so the tail is visible.
const T_SIM_MAX = 120

// Seed-picked stage-2 mystery plasmas and stage-3 hidden plasmas.
const STAGE2_TARGETS = [8, 12, 18, 25, 32]
const STAGE3_TARGETS = [10, 15, 22, 28, 35]

// Tolerances.
const STAGE2_TOL = 0.05 // ±5% on |Tstudent − Ttarget| / Ttarget
const STAGE3_TOL = 0.03 // ±3% on the numeric input

// Sample fractions of T½ᶠᵘˢ: one before, one at, one after the half-life.
const STAGE2_SAMPLE_FRACTIONS = [0.5, 1.0, 2.0]
const STAGE3_SAMPLE_FRACTIONS = [0.6, 1.2, 2.0, 2.8]

// Gaussian noise applied to Evaluate's reactor readings (relative to N₀).
const STAGE3_NOISE_SIGMA = 0.04

// Energy released per D–T fusion event (MeV) — the pedagogical Q hook.
const Q_PER_FUSION_MEV = 17.6
...
/** From N0 uniform draws U∈(0,1], derive sorted fusion-time lifetimes for a given λ. */
...
/** Count of D–T pairs still un-fused at time t given sorted fusion times. */
...
export default function Scene() {
  ...
  // Fork 1 — PHYSICS stream: N0 uniform draws for lifetime remapping.
  // MUST be the first fork call. Never touched by cosmetic code.
  ...
  // Fork 2 — target plasma pick via seed modulo (no fork consumption).
  ...
  // Fork 3 — NOISE stream: Gaussian jitter on stage-3 reactor readings.
  // Double-fork advances past target-fork position so physics stream stays untouched.
  const stage3Points = useMemo(() => {
    ...
    return STAGE3_SAMPLE_FRACTIONS.map((frac) => {
      ...
      const n = Math.max(
        0,
        Math.min(N0, nMean + N0 * noiseRng.gauss(0, STAGE3_NOISE_SIGMA)),
      )
  ...
  // Sorted fusion times for the current T (analytic remap of the shared U samples).
  ...
  const nFused = N0 - nAlive
  const qReleased = nFused * Q_PER_FUSION_MEV // MeV
  const rate = lambdaFromT(T) * nAlive // fusion events per u
  ...
  // Ticker — sim runs while running & tSim < T_SIM_MAX. Killed on stage 3.
  useTicker((dt) => {
    if (isStage3) return
    if (!running) return
    setTSim((prev) => {
  ...
  for (let i = 0; i < N0; i++) {
    ...
    // "Just fused" flash: within 0.6u after fusion time
    const justFused = !alive && tau > tSim - 0.6
    dots.push(
      <circle
        key={i}
        cx={cx}
        cy={cy}
        r={dotR}
        fill={alive ? '#37C9B8' : justFused ? '#F97316' : '#2A3244'}
        stroke={alive ? '#37C9B8' : justFused ? '#F97316' : '#3A4863'}
        strokeWidth={alive ? 0 : 0.6}
        opacity={alive ? 0.95 : justFused ? 0.7 : 0.35}
      />,
    )
  }
  ...
  // TR is composed as up to 3 stacked lines. BR is left empty (parent chrome).
  const hudTRLine1 = isStage3
    ? peekVisible
      ? `${labels.peek_reveal} T½ = ${targetT3} u`
      : `T½ = ?`
    : `λ = ${lambdaFromT(T).toFixed(3)} /u`
  const hudTRLine2 = isStage1
    ? `N(t) = ${nAlive} · R = ${rate.toFixed(2)} /u`
    : isStage2
      ? `Δ = ${(100 * Math.abs(T - targetT2) / targetT2).toFixed(1)}%`
      : ''
  const hudTRLine3 = isStage1
    ? `Q = ${qReleased.toFixed(1)} MeV`
    : ''
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      >
        ...
        {/* ─── Left panel: plasma dot grid (Stages 1 & 2) ────────────── */}
        {!isStage3 && (
          <>
            <rect
              x={GRID_X - 8}
        ...
        {/* ─── Left panel replacement on Stage 3: reactor readings card ─── */}
        {isStage3 && (
          <>
            <rect
              x={GRID_X - 8}
        ...
        {/* T½ᶠᵘˢ vertical marker (only on stages 1 & 2 — training wheel) */}
        {!isStage3 && (
          <>
            <line
              x1={tToSvgX(T)}
        ...
        {/* Stage 3: reactor-reading data points + drop-lines */}
        {isStage3 &&
          stage3Points.map((p, i) => (
            <g key={`s3p${i}`}>
              <circle
                cx={tToSvgX(p.t)}
                cy={nToSvgY(p.n, logScale)}
                r={5}
                fill="#F97316"
              />
              <line
        ...
        {/* Stage 3 peek: 1.5 s reveal of the true depletion curve.
            Shows the shape the student is fitting to — NOT the T½ value. */}
      ...
      {/* TR — stacked lines: λ / (N,R or Δ%) / Q. Live readouts live here,
          NEVER in BR (which the fullscreen toggle owns). */}
      <div
        style={{
          position: 'absolute',
          top: '3rem',
      ...
      >
        <div>{hudTRLine1}</div>
        {hudTRLine2 && (
          <div style={{ fontSize: '1.9rem', color: '#F9A968' }}>{hudTRLine2}</div>
        )}
        {hudTRLine3 && (
          <div style={{ fontSize: '1.9rem', color: '#F9A968' }}>{hudTRLine3}</div>
        )}
      </div>
      <div
        style={{
      ...
      {/* Stage 3 only: numeric input + submit near the bottom center.
          Kept clear of the BR quadrant (reserved for parent chrome). */}
      {isStage3 && (
        <div
          style={{
            position: 'absolute',
